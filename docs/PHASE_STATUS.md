...
**Deferred to a follow-up session (not started):**

* Resizable 22/39/39 Outline/PDF/Markdown panel layout, filling the viewport instead of the current fixed `h-[640px]` pane — plan is to add `react-resizable-panels` as one justified new dependency.
* Focus Mode (F11/double-click/toolbar) collapsing side rails to icon strips, plus the two missing split-view presets (PDF+DOCX, Markdown+DOCX) in `WorkspaceShell.tsx`.
* A numbered Reading Order overlay directly on `PdfViewer.tsx` (currently a separate list-only panel with up/down reorder buttons).
* Persistent Validation Issue backend — `src/models/validation_issue.py` needs `issue_id`/`status`/timestamps + a `ValidationIssueStatus` enum, and a new `PATCH /documents/{job_id}/validation-issues/{issue_id}` endpoint mirroring `review_correction`'s pattern, kept status-only (no document mutation, no version bump) per the model's existing "read-only side-channel" docstring. Decision to build this (rather than defer it like Settings/table-merge-split) recorded in `DECISIONS_LOG.md` Part 25.
