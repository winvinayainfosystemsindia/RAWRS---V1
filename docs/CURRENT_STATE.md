...
A FastAPI backend (`src/api/main.py`, `routes.py`, `jobs.py`, `schemas.py`) and a Next.js/React/TypeScript/Tailwind frontend (`frontend/`) both exist and are functional — confirmed by direct inspection and by exercising the backend over real HTTP. This differed from the originally planned Vite/Zustand/shadcn stack; on 2026-09-16 the built stack was decided as authoritative and `docs/TECH_STACK.md` was updated to match (`DECISIONS_LOG.md` Part 25).

What's actually built: an upload page, and a per-document `WorkspaceShell` (`frontend/components/workspace/`) — **this superseded the old tab-per-object-type layout at Phase M-2 (see `PHASE_STATUS.md`); this paragraph's prior "workspace page with tabs for Validation, Headings, Images..." framing was accurate through FEATURE_016 but is not accurate now.** Current structure: a `SemanticNavTree` left rail (Outline / By Type / Pending / Issues / Search modes, plus a "Workspaces" section linking to whole-document editors — Images, Tables, Headings, Metadata, OCR Pages, Reading Order, Page Labels, Corrections, Accessibility Readiness, Validation), a persistent PDF/Markdown/DOCX center-pane switcher, a `ContextInspectorRail` + `ObjectInspectorFrame` right rail driven by object selection, and a collapsible `BottomPanel`. Download buttons for the markdown/docx/report files remain available via the Corrections/Export area. **FEATURE_012 (2026-06-28):** The Images workspace has full human review actions — per-image card with Generate AI Alt Text / Approve / Reject / Mark Decorative / Mark Complex / Skip / Edit buttons, an ImageDetailPanel sidebar showing AI structured output (Description / Purpose / Visible Text / Confidence / Warnings), and a BulkActions toolbar for multi-select operations. The DOCX download re-generates the DOCX from current in-memory state when any image has been reviewed, so approved alt texts flow into the downloaded file. Three new API endpoints: `POST generate-alt-text`, `PATCH /images/{id}`, `POST /images/bulk-action`. **FEATURE_015 (2026-06-29):** A Tables workspace auto-detects tables with visible PDF borders via PyMuPDF `find_tables(strategy='lines')` and renders them as pipe tables in Markdown and `docx.add_table()` in DOCX. Reviewers can add caption text and a WCAG H73 accessibility summary per table, toggle header rows via the TableDetailPanel sidebar, and manually create tables for borderless academic tables the detector missed. Four new API endpoints: `GET /tables`, `POST /tables`, `PATCH /tables/{id}`, `DELETE /tables/{id}`. Job tracking is in-memory only (`src/api/jobs.py`) and does not survive a process restart.

**Phase 1 IDE redesign, 2026-07-08/09 (5 of 9 planned items shipped — see `PHASE_STATUS.md`'s "Phase 1 IDE Redesign" section and `DECISIONS_LOG.md` Part 25 for the full breakdown and the Live Projection Model decision):** DocxPreview/Markdown were silently going stale after any edit (a real bug — `DocumentProvider`'s polling stopped once a job completed, so `document_version` froze client-side) — fixed, both now stay live-synced. `TableGrid.tsx`/`HeadingGrid.tsx` (previously built but never wired into any nav) are now reachable as dedicated Workspaces. `ImageGrid` gained filters (Missing Alt/Needs Review/Accepted/Rejected/Decorative/Low Resolution) and whole-document bulk AI generation. `ObjectInspectorFrame` is now tabbed (Properties/Evidence/History/AI/Actions) instead of one long scrolling panel. Remaining: resizable panel layout, Focus Mode, a Reading Order PDF overlay, and backend persistence for validation-issue review status (not yet started).
...
**Update: `fastapi`/`uvicorn`/`python-multipart` are now present** — this corrects this file's own prior claim that they were absent. The frontend's actual dependencies (`frontend/package.json`, re-checked 2026-09-16): `next`, `react`, `react-dom`, `react-pdf`, `react-resizable-panels`, `codemirror` + `@codemirror/lang-markdown`, `mammoth`; dev: `tailwindcss`, `typescript`, `eslint`, `jest` + Testing Library + `jest-axe`. `docs/TECH_STACK.md` now describes this stack (reconciled in `DECISIONS_LOG.md` Part 25).
