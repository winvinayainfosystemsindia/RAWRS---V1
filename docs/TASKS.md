...
* [x] A-1 — Accessibility Intelligence Engine Design (`docs/ACCESSIBILITY_INTELLIGENCE_ENGINE_DESIGN.md`, new; `docs/PHASE_STATUS.md` "Phase A-1"), the backend engine RW-1 named as deferred. Design only, no code: rule architecture/registry/lifecycle/pipeline built on `src/verification/`'s already-proven `SemanticVerifier` registration pattern and `EvidenceSignal`/`EvidenceBundle` confidence model; a single-axis Barrier/Degradation/Observation weighting (10/5/2 points, chosen over a rejected WCAG-level-multiplier draft); a full rule taxonomy classifying every existing `validator.py` rule_id as scored (real WCAG/PDF-UA citation) or observational (no fabricated mappings); concrete new rules (`READING_ORDER_002`, `NAV_001`, `LANG_001/002`) closing the exact 3 categories `validationCategories.ts` already lists as deferred, with `LANG_002` honestly flagged as blocked on a detection capability that doesn't exist yet rather than assumed solvable. Reconciled against Design Bible §17 (no new "Accessibility View" screen — confirmed no conflict) and §10 (AI Confidence slider/running score — this design supplies the `RuleEvaluation.confidence` data shape). 4-phase roadmap with backward-compatible `GET /readiness` adapter so `ReadinessPanel.tsx` needs zero changes for Phases 1-3. Refinement pass added 4 reviewer-guidance/reporting sections (Accessibility Impact, Predicted Score, Accessibility Debt, Rule Provenance), each extending the approved architecture (category-level `RuleImpact`, `predict_score()` reusing the existing scoring function, debt classes mapped onto the existing `BarrierClass`, provenance assembled from existing fields plus one backward-compatible `EvidenceSignal.source_module` addition) — zero breaking changes, zero new registries. **Stopped for approval per the mission's explicit instruction — not implemented.**
* [x] A-2 — Accessibility Intelligence Engine, Phase 1 Implementation (`docs/PHASE_STATUS.md` "Phase A-2"), approved and built: `src/accessibility/` new module (models/registry/scoring/pipeline/debt/provenance + 14 rules across headings/images/tables/metadata/reading-order), additive `GET /documents/{job_id}/accessibility-report` endpoint, one backward-compatible `EvidenceSignal.source_module` field addition. 54 new tests, full suite 1608 passed/0 failed (zero regressions). Code-reviewer pass caught and fixed 2 real HIGH-severity bugs (AI-generated alt text silently passing as human-reviewed; a confidence-gate rule incorrectly blocking export) plus 1 MEDIUM completeness gap (TABLE_007 folding), all before being called done. Deliberately deferred, disclosed not hidden: `readiness.py`/`get_export_readiness` adapters (Section 22 roadmap items 3/5 — too risky to migrate without dedicated before/after regression coverage on a live endpoint), `ManualAttestation` generalization, AI-assisted rules, `*_VERIFY_*` cross-source folding, frontend consumption.
